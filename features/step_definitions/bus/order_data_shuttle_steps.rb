When /^I order data shuttle for (.+)$/ do |search_key|
  ...
  @bus_admin_console_page.order_data_shuttle_section.search_partner(search_key)
  @bus_admin_console_page.order_data_shuttle_section.process_order(search_key)
end
...
Then /^Order data shuttle error message should match: (.+)$/ do |err_message|
  @bus_admin_console_page.process_order_section.message_text.should match(err_message)
end

Then /^Order data shuttle successful message should match: Data Shuttle Device for Pro Partner (.+) created\.$/ do |company_name|
  @bus_admin_console_page.process_order_section.message_text.should == "Data Shuttle Device for Pro Partner #{company_name} created."
end
...
When /^I search data shuttle order by (.+)$/ do |company_name|
  ...
  @bus_admin_console_page.view_data_shuttle_orders_section.search_order(company_name)
...
Then /^The number of driver in shipping tracking table should be (\d+)$/ do |num_drivers|
  @bus_admin_console_page.order_details_section.shipping_tracking_tb_rows_text.length.should == num_drivers.to_i
end