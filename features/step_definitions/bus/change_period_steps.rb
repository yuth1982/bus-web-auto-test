...
When /^I change subscription up to (.+)$/ do |link_text|
  @bus_admin_console_page.billing_info_view.navigate_to_change_period_view
...
Then /^Change subscription confirmation message should include (.+)$/ do |message|
  @bus_admin_console_page.change_period_view.change_confirmation_text.should include(message)
end

Then /^Change subscription price table should be:$/ do |price_table|
  @bus_admin_console_page.change_period_view.price_tb_header_text.should == price_table.headers
  @bus_admin_console_page.change_period_view.price_tb_rows_text.should == price_table.rows
end

Then /^Subscription changed message should be (.+)$/ do |message|
  @bus_admin_console_page.change_period_view.message_text.should == message
end